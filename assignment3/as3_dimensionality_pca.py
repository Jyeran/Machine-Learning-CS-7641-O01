...
for i in range(1,10):
    ...
    for d in range(1,6):
        ...
        dt.fit(pc_train[:,:i], y_train)
        
        y_test_pred  = dt.predict(pc_test[:,:i])
...
p1, = host.plot(crange, ev, "b-", label="Eigenvalue")
p2, = par1.plot(crange, evCum, "r-", label="Cumulative Variance")
p3, = par2.plot(crange, dtAcc, "g-", label="Cross Validdation DT Accuracy")

host.set_xlabel("Principle Components")
host.set_ylabel("Eigenvalue")
par1.set_ylabel("Cumulative Variance")
par2.set_ylabel("Decision Tree Cross Val Accuracy")

host.yaxis.label.set_color(p1.get_color())
par1.yaxis.label.set_color(p2.get_color())
par2.yaxis.label.set_color(p3.get_color())

fig.suptitle("WINE: PCA Variance and DT Accuracy by # of Components")
plt.show()
plt.close()
...
for k in krange:
  ...
  if k >= 2:
      kmeans = KMeans(n_clusters = k).fit(pc_train[:,:3])
      labels = kmeans.labels_
      sil.append(silhouette_score(pc_train[:,:3], labels, metric = 'euclidean'))
...
for k in krange:
    ...
    gmm.fit(pc_train[:,:3])
    labels = gmm.predict(pc_train[:,:3])
    logLpca.append(gmm.score(pc_train[:,:3]))
...
fig.suptitle("WINE: EM Clusters by SSE and Silhoouette", fontsize=20, color = 'purple')
plt.show()
plt.close()
...
kmeans = KMeans(n_clusters = 2).fit(pc_train[:,:3])
Klabels = kmeans.labels_

gmm = GaussianMixture(n_components = 4)
gmm.fit(pc_train[:,:3])
Elabels = gmm.predict(pc_train[:,:3])
...
plt.title("WINE: Percentage of High Quality Wine by Kmeans Cluster: PCA", color = 'purple')
plt.xlabel("Cluster")
...
plt.title("WINE: Percentage of High Quality Wine by EM Cluster: PCA", color = 'purple')
plt.xlabel("Cluster")
plt.ylabel("Percent of High Quality Wine")
plt.show()
plt.close()
...
for n_clusters in range_n_clusters:
    ...
    for i, c in enumerate(centers):
        ax2.scatter(c[0], c[1], marker="$%d$" % i, alpha=1, s=50, edgecolor="k")
        """
    ax2.set_title("WINE: EM Cluster Visualization", color = 'purple')
    ax2.set_xlabel("Feature space for the 1st feature")
    ax2.set_ylabel("Feature space for the 2nd feature")
...
fig.suptitle("CANCER: PCA Variance and DT Accuracy by # of Components")
plt.show()
plt.close()
...
plt.plot(krange,logLpca, label = 'EM PCA')
plt.legend()
plt.title('CANCER: K Clusters by Log Likelihood', color = 'r')
plt.xlabel('K-Value')
plt.ylabel('Log Likelihood')
plt.show()
plt.close()
...
fig.suptitle("CANCER: K Clusters by SSE and Silhoouette", fontsize=20, color = 'r')
plt.show()
plt.close()
